...
export function readCustomAssetMeta(relativePath) {
...
export function readCustomAsset(relativePath) {
  const meta = readCustomAssetMeta(relativePath);
  if (!meta) {
    return null;
  }
  const buffer = fs.readFileSync(meta.abs);
  return {
    buffer,
    hash: meta.hash,
...
export function readCustomAssetIfHashMatches(hash, relativePath) {
  ...
  const asset = readCustomAssetMeta(relativePath);
