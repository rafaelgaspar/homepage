...
import { metricsContentType, renderMetrics } from "./registry";
...
export function startMetricsServer({ port: portOverride, host = "0.0.0.0" } = {}) {
  ...
  server = http.createServer(async (req, res) => {
    try {
      ...
      logger.error("Metrics request failed: %s", err.message || err);
      if (!res.headersSent) {
        res.statusCode = 500;
  ...
  server.listen(port, host, () => {
    logger.info("Prometheus metrics listening on %s:%d%s", host, port, metricsPath);
  });

  server.on("error", (err) => {
    logger.error("Metrics server error: %s", err.message || err);
  });
